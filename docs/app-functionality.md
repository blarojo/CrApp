...
- Insights shortcut (upload a report generated by the `crapp-insights` skill) — see
  [§13](#13-insights-ai-generated-report-v2). Also reachable from a top-level link
  on Home next to History/Export. ✅
...
## 13. Insights (AI-generated report) *(v2)*

Top-level **Insights** link on Home (next to History and Export) — also still
reachable via Settings → Insights, unchanged. See
[future-features.md spec 11](future-features.md#11-ai-generated-insights-v2-progression--moodfood-correlation)
for the full design and `.claude/skills/crapp-insights/SKILL.md` for the
...
- Nothing computes on-device: the screen uploads a JSON report file produced
  *outside* the app by the `crapp-insights` Claude skill from a fresh CSV export
  (Export screen → share the CSVs → run the skill → upload the JSON it writes back
  here). No network calls, no on-device inference. ✅ (upload/parse/render path)
  🧪 (not yet exercised with a real skill-generated report)
...
- A ⚠️ data-completeness caveat line (e.g. noting dog-walker-reported walks have no
  ...
  one. 🧪
- Below the grouped findings: the existing generic line/bar trend-series charts
  (consistency over time, movements per week, etc.), unchanged from v1.
